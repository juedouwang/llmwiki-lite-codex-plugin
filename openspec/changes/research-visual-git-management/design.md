...
### D-12 2026-09-20 用户细节修订

- 代码SVG为细线三圆圈分叉；合并也补全主干末端。右栏原型布局保持，唯diff增加只读放大入口，底部操作改为紧跟diff（不再 `margin-top:auto`）。元数据从commit API的 `repository_path` 取实际仓库路径；放大复用同一已读diff，不触发写入或请求远端。
- `/` 仅作为启动入口；`/projects` 为可管理的总览，品牌及项目切换菜单的“管理项目”均指向它。选定具体项目URL或浏览器恢复深链接不被强行改写。默认项目与助手选中项独立，新字段不迁移旧current值。
- `GET/POST /api/projects/preferences` 沿用本地Host/同源JSON请求验证；POST仅接受 `default_project_id`（ID或null）和/或 `project_order`（全部现有注册ID的无重复排列）。写入持有既有home锁并原子替换settings，校验失败不部分写入；返回有效默认、排序和落地项目ID。注册项目无预排序时仍按名字；已排序ID优先，后注册项目在末尾；注销清理两个新字段。
- `projects.js` 使用文件夹按钮上的Pointer Events拖动与上下键，松开自动保存；保存中避免丢失设置、失败恢复顺序。项目名称链接与手柄分开，无额外拖拽装饰。显式默认用可取消的星标，只有一个；偏好事件刷新当前及已缓存侧栏菜单顺序，不销毁Git/笔记DOM。
- 旧HTML源稿保持只读；对照测试继续验证未改布局，用户本轮明确修改的diff控件偏移和按钮底部位置改用新浏览器断言，不人为修改旧原型令比较通过。
