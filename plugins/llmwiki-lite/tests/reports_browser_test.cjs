...
(async () => {
  ...
  try {
    ...
    await nav.getByRole('link',{name:'日报与周报',exact:true}).click();
    ...
    await nav.getByRole('link',{name:'周报',exact:true}).click();
    await expectProject(nav, cfg.pid, '报告交互验收');
    await nav.getByLabel('筛选参与项目').selectOption(cfg.other_pid);
    await nav.getByLabel('搜索报告').fill('2026-09-14');
    ...
    await nav.reload();
    await expectProject(nav, cfg.pid, '报告交互验收');
    await nav.getByRole('link',{name:'返回列表',exact:true}).click();
    ...
    await nav.locator('.console-project-menu').getByRole('link',{name:'第二项目',exact:true}).click();
    await expectProject(nav, cfg.other_pid, '第二项目');
    ...
    assert.equal(await nav.getByLabel('搜索报告').inputValue(), '2026-09-14');
    await nav.getByRole('link',{name:'科研记录',exact:true}).click();
    ...
    await expectProject(nav, cfg.pid, '报告交互验收');
    await nav.getByRole('link',{name:'日报',exact:true}).click();
    await expectProject(nav, cfg.pid, '报告交互验收');
    ...
    assert.equal(await nav.locator('.console-project-label b').innerText(), '选择项目');
    await nav.getByLabel('切换研究项目',{exact:true}).click();
    await nav.locator('.console-project-menu').getByRole('link',{name:'报告交互验收',exact:true}).click();
    await expectProject(nav, cfg.pid, '报告交互验收');
    await nav.close();
    ...
    await page.getByLabel('筛选参与项目').selectOption(cfg.other_pid);
    ...
    await page.getByRole('link', {name:'日报与周报', exact:true}).click();
    assert.equal(new URL(page.url()).pathname, '/reports');
    assert.equal(await page.locator('.console-navigation a[aria-current=page]').innerText(), '日报与周报');
    await page.getByRole('link', {name:/日报 · 2026-09-19/}).click();
