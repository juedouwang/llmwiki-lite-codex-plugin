"""Tests for git_revert_restore.py (T-10).

Tests acceptance criteria:
- AT-45: restore_tree to R0, new commit tree = R0 tree, history reachable
- AT-46: revert regular and merge commits, mainline required for merge
- AT-47: published/stale state blocks reset, guides to restore_tree
- AT-48: valid local reset or restore file with recovery point
- AT-49: special repo/timeout handling, no blind retry

Fixtures: GIT-F1 (regular fork), GIT-F5 (merge commits)
"""
...
import sys
sys.path.insert(0, str(Path(__file__).parent.parent / "plugins" / "llmwiki-lite" / "scripts"))
...
@pytest.fixture
def git_f1_repo(temp_env, git_exe):
    """GIT-F1 fixture repository."""
    repo_dir = temp_env["source"] / "test-repo"
    repo_dir.mkdir()
    commits = create_git_f1_fixture(repo_dir, git_exe)
    return {"repo_dir": repo_dir, "commits": commits}


@pytest.fixture
def merge_repo(temp_env, git_exe):
    """Repository with merge commit."""
    repo_dir = temp_env["source"] / "merge-repo"
    repo_dir.mkdir()
    commits = create_merge_commit_fixture(repo_dir, git_exe)
    return {"repo_dir": repo_dir, "commits": commits}
...
def test_restore_tree_creates_new_commit_with_target_tree(git_f1_repo, temp_env, git_exe):
    """AT-45: restore_tree to R0, new commit tree = R0 tree, M1/M2 history reachable."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]
    ...
    analysis = analyze_restore_tree(repo_dir, git_exe, commits["R0"])
    ...
    # Get R0 tree OID
    result = subprocess.run(
        [git_exe, "rev-parse", f"{commits['R0']}^{{tree}}"],
        cwd=repo_dir,
        capture_output=True,
        text=True,
        check=True
    ...
    receipt = restore_tree(
        ...
        state_root=temp_env["state"],
        git_exe=git_exe,
        repo_id="test-repo",
        worktree_id="main-worktree",
        target_oid=commits["R0"],
        lock_dir=temp_env["lock"]
    )
    ...
    result = subprocess.run(
        [git_exe, "rev-parse", f"{new_commit}^{{tree}}"],
        cwd=repo_dir,
        capture_output=True,
        text=True,
        check=True
    ...
    m1_reachable = subprocess.run(
        [git_exe, "merge-base", "--is-ancestor", commits["M1"], "HEAD"],
        cwd=repo_dir
    )
    assert m1_reachable.returncode == 0, "M1 must still be reachable"

    m2_reachable = subprocess.run(
        [git_exe, "merge-base", "--is-ancestor", commits["M2"], "HEAD"],
        cwd=repo_dir
    )
...
def test_restore_tree_no_change_when_trees_match(git_f1_repo, temp_env, git_exe):
    """AT-45: restore_tree returns no_change when current tree already matches target."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]

    # Restore to current commit (M2)
    analysis = analyze_restore_tree(repo_dir, git_exe, commits["M2"])
...
def test_restore_tree_blocks_untracked_conflicts(git_f1_repo, temp_env, git_exe):
    ...
    subprocess.run([git_exe, "checkout", commits["R0"]], cwd=repo_dir, check=True)
    (repo_dir / "new-file.txt").write_text("tracked\n", encoding="utf-8")
    subprocess.run([git_exe, "add", "new-file.txt"], cwd=repo_dir, check=True)

    env = os.environ.copy()
    env["GIT_AUTHOR_DATE"] = "2026-01-06T00:00:00Z"
    env["GIT_COMMITTER_DATE"] = "2026-01-06T00:00:00Z"
    subprocess.run([git_exe, "commit", "-m", "Add new file"], cwd=repo_dir, env=env, check=True)
    ...
    analysis = analyze_restore_tree(repo_dir, git_exe, new_commit)
    assert "new-file.txt" in analysis.untracked_conflicts

    # restore_tree should fail
    with pytest.raises(GitOperationError, match="Untracked files would be overwritten"):
        restore_tree(
            worktree_root=repo_dir,
            state_root=temp_env["state"],
            git_exe=git_exe,
...
def test_revert_regular_commit(git_f1_repo, temp_env, git_exe):
    """AT-46: Revert regular commit creates reverse commit."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]

    # Analyze revert of M2
    analysis = analyze_revert(repo_dir, git_exe, commits["M2"])
...
def test_revert_merge_commit_requires_mainline(merge_repo, temp_env, git_exe):
    """AT-46: Merge commit revert without mainline must not execute."""
    repo_dir = merge_repo["repo_dir"]
    commits = merge_repo["commits"]

    # Analyze M3 (merge commit)
    analysis = analyze_revert(repo_dir, git_exe, commits["M3"])
    assert analysis.is_merge
    assert len(analysis.parents) == 2

    # Attempt revert without mainline - must fail
    with pytest.raises(MainlineRequiredError, match="Must specify which parent to keep"):
        start_revert(
            worktree_root=repo_dir,
            state_root=temp_env["state"],
            git_exe=git_exe,
...
def test_revert_merge_commit_with_mainline(merge_repo, temp_env, git_exe):
...
def test_abort_revert(git_f1_repo, temp_env, git_exe):
    ...
    (repo_dir / "config.txt").write_text("value=main\nconflict=yes\n", encoding="utf-8")
    subprocess.run([git_exe, "add", "config.txt"], cwd=repo_dir, check=True)
    subprocess.run([git_exe, "commit", "-m", "Add conflict"], cwd=repo_dir, check=True)
    ...
    result = subprocess.run(
        [git_exe, "revert", "--no-commit", commits["M2"]],
        cwd=repo_dir,
        capture_output=True
    )
...
def test_reset_blocks_published_commits(git_f1_repo, temp_env, git_exe):
    """AT-47: Reset blocks when commits are published (reachable from remote refs)."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]

    # Create a bare remote
    remote_dir = temp_env["source"] / "remote.git"
    subprocess.run([git_exe, "init", "--bare"], cwd=remote_dir, check=True)

    # Add remote and push
    subprocess.run([git_exe, "remote", "add", "origin", str(remote_dir)], cwd=repo_dir, check=True)
    subprocess.run([git_exe, "push", "-u", "origin", "main"], cwd=repo_dir, check=True)

    # Now M2 is published
    is_published, check_time, is_stale = check_commit_published(repo_dir, git_exe, commits["M2"])
    assert is_published

    # Attempt to reset to M1 - should fail
    analysis = analyze_reset(repo_dir, git_exe, commits["M1"])
    assert analysis.is_published

    with pytest.raises(PublishedCommitError, match="commits are published"):
        reset_branch(
            worktree_root=repo_dir,
            state_root=temp_env["state"],
            git_exe=git_exe,
...
def test_reset_allows_unpublished_local_commits(git_f1_repo, temp_env, git_exe):
...
def test_reset_blocks_when_remote_refs_stale(git_f1_repo, temp_env, git_exe):
    ...
    subprocess.run([git_exe, "add", "new-file.txt"], cwd=repo_dir, check=True)
    subprocess.run([git_exe, "commit", "-m", "New local commit"], cwd=repo_dir, check=True)
...
def test_restore_file_from_commit(git_f1_repo, temp_env, git_exe):
...
def test_restore_file_removes_when_not_in_target(git_f1_repo, temp_env, git_exe):
    """AT-48: Restore file removes file when it doesn't exist in target."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]

    # Switch to experiment branch which has experiment.txt
    subprocess.run([git_exe, "checkout", "experiment"], cwd=repo_dir, check=True)
...
def test_reset_creates_recovery_point(git_f1_repo, temp_env, git_exe):
    ...
    recovery_dir = temp_env["state"] / "workbench" / "git-recovery" / receipt.recovery_id
...
def test_reset_blocks_dirty_worktree(git_f1_repo, temp_env, git_exe):
    ...
    with pytest.raises(GitDirtyWorktreeError, match="must be clean"):
        reset_branch(
            worktree_root=repo_dir,
            state_root=temp_env["state"],
            git_exe=git_exe,
...
def test_reset_blocks_detached_head(git_f1_repo, temp_env, git_exe):
    """AT-48: Reset not allowed from detached HEAD."""
    repo_dir = git_f1_repo["repo_dir"]
    commits = git_f1_repo["commits"]

    # Detach HEAD
    subprocess.run([git_exe, "checkout", commits["M2"]], cwd=repo_dir, check=True)
    ...
    with pytest.raises(GitOperationError, match="detached HEAD"):
        analyze_reset(repo_dir, git_exe, commits["M1"])
...
def test_check_repository_constraints_shallow(temp_env, git_exe):
    """AT-49: Detect shallow clone."""
    repo_dir = temp_env["source"] / "shallow-repo"
    repo_dir.mkdir()
    ...
    subprocess.run([git_exe, "add", "."], cwd=repo_dir, check=True)
    subprocess.run([git_exe, "commit", "-m", "Initial"], cwd=repo_dir, check=True)

    # Simulate shallow by creating shallow file
    shallow_file = repo_dir / ".git" / "shallow"
    shallow_file.write_text("dummy\n", encoding="utf-8")

    constraints = check_repository_constraints(repo_dir, git_exe)
    assert constraints["is_shallow"]
    assert any("Shallow clone" in w for w in constraints["warnings"])


def test_check_repository_constraints_submodules(temp_env, git_exe):
    """AT-49: Detect submodules."""
    repo_dir = temp_env["source"] / "submodule-repo"
    repo_dir.mkdir()
...
def test_restore_tree_rejects_submodule_affected_operations(temp_env, git_exe):
    ...
    repo_dir = temp_env["source"] / "sub-repo"
    repo_dir.mkdir()
...
def test_full_revert_restore_reset_workflow(temp_env, git_exe):
    """Integration test: revert, restore_tree, and reset operations."""
    repo_dir = temp_env["source"] / "workflow-repo"
    repo_dir.mkdir()
