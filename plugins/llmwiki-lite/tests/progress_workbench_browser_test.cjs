...
(async()=>{
  ...
  try{
    ...
    const currentDate=await page.evaluate(()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;});
    const later=n=>{const d=new Date(currentDate+'T12:00:00Z');d.setUTCDate(d.getUTCDate()+n);return d.toISOString().slice(0,10);};
    const bar=title=>page.locator(`#progress-timeline .progress-bar[aria-label^="${title} · "]`);
    const columns=title=>bar(title).evaluate(e=>[e.style.gridColumnStart,e.style.gridColumnEnd]);
    assert.match(await page.locator('#progress-range-label').innerText(),new RegExp('^'+currentDate));
    assert.deepEqual(await columns('中优先级今天截止'),['1','2']);
    assert.deepEqual(await columns('低优先级明天截止'),['1','3']);
    assert.equal(await bar('高优先级未排期').count(),0);
    assert.equal(await bar('旧已完成').count(),0);
    let beforeRange=await data();
    const added=await context.request.post(api,{headers:{'X-Notebook-Request':'1','Origin':cfg.origin},data:{action:'create',revision:beforeRange.revision,task:{title:'跨周长任务',priority:'high',ddl:later(10)}}});
    assert.equal(added.status(),200);
    await page.reload();await bar('跨周长任务').waitFor();
    assert.deepEqual(await columns('跨周长任务'),['1','8']);
    assert.equal(await bar('跨周长任务').evaluate(e=>e.classList.contains('is-clipped-end')),true);
    const baselineRange=(await data()).tasks;
    await page.locator('#progress-next').click();
    assert.deepEqual(await columns('跨周长任务'),['1','5']);
    assert.equal(await bar('跨周长任务').evaluate(e=>e.classList.contains('is-clipped-start')),true);
    await page.locator('#progress-today').click();
    await page.locator('#progress-days').selectOption('14');
    assert.deepEqual(await columns('跨周长任务'),['1','12']);
    await page.locator('#progress-days').selectOption('28');
    assert.deepEqual(await columns('跨周长任务'),['1','12']);
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false);
    await page.locator('#progress-timeline').evaluate(e=>e.scrollLeft=e.scrollWidth);
    const sticky=await page.locator('.progress-grid-row:not(.progress-grid-head) .progress-calendar-title').first().boundingBox();
    const timelineBox=await page.locator('#progress-timeline').boundingBox();assert.ok(Math.abs(sticky.x-timelineBox.x)<3);
    await page.locator('#progress-days').selectOption('7');
    await page.locator('#progress-prev').click();assert.equal(await page.locator('#progress-timeline .progress-bar').count(),0);
    await page.locator('.progress-outside').click();assert.deepEqual(await columns('低优先级明天截止'),['1','3']);
    assert.deepEqual((await data()).tasks,baselineRange);
    const colors=await page.locator('.progress-bar:not(.is-overdue)').evaluateAll(es=>es.map(e=>getComputedStyle(e).backgroundColor));
    assert.equal(new Set(colors).size,3);
    const borders=await page.locator('.progress-bar').evaluateAll(es=>es.map(e=>getComputedStyle(e).borderLeftWidth));
    assert.ok(borders.every(width=>width==='0px'));
    await screenshot('progress-span-light.png');
    await page.emulateMedia({colorScheme:'dark'});await screenshot('progress-span-dark.png');await page.emulateMedia({colorScheme:'light'});
    checks.push('today-to-deadline spans; cross-week clipping; 7/14/28 days; sticky labels; colors; read-only range navigation');

    await page.locator('#progress-new').click();
    const priorityBox=await form.locator('[name=priority]').boundingBox(),dateBox=await form.locator('[name=ddl]').boundingBox();
    assert.ok(Math.abs(priorityBox.y-dateBox.y)<1);assert.ok(Math.abs(priorityBox.height-dateBox.height)<1);assert.ok(Math.abs(priorityBox.width-dateBox.width)<1);
    const shortcutsBox=await form.locator('.progress-date-shortcuts').boundingBox();assert.ok(Math.abs(shortcutsBox.x-dateBox.x)<1);assert.ok(shortcutsBox.y>dateBox.y+dateBox.height);
    await form.locator('.progress-task-options').screenshot({path:path.join(cfg.output,'progress-fields-aligned.png')});
    ...
    assert.equal(await page.locator('#progress-timeline .progress-bar[aria-label^="完整新任务"]').count(),1);
    ...
    for(const width of [480,320]){
      await page.setViewportSize({width,height:900});await page.locator('#progress-new').click();
      const priorityBox=await form.locator('[name=priority]').boundingBox(),dateBox=await form.locator('[name=ddl]').boundingBox();
      assert.ok(Math.abs(priorityBox.x-dateBox.x)<1);assert.ok(Math.abs(priorityBox.width-dateBox.width)<1);assert.ok(dateBox.y>priorityBox.y+priorityBox.height);
      assert.equal(await form.evaluate(e=>e.scrollWidth<=e.clientWidth+1),true);
      await screenshot(`progress-dialog-mobile-${width}.png`);await close();
    }
    // Saving visible selections must remove stale configuration, not access the removed project.
    await page.goto(cfg.origin+'/settings');await page.locator('#report-settings').evaluate(e=>e.open=true);
    await page.waitForFunction(()=>document.querySelector('#report-capture').checked);
    const configBefore=await (await context.request.get(cfg.origin+'/api/reports/settings')).json();
    assert.ok(!configBefore.project_ids.includes(cfg.removedProjectId));
    assert.ok(!Object.hasOwn(configBefore.activity_db_paths,cfg.removedProjectId));
    assert.ok((await page.locator('#report-settings > .meta').innerText()).includes('新注册项目默认参与'));
    assert.equal(await page.locator(`input[name=report-project][value="${cfg.removedProjectId}"]`).count(),0);
    await page.getByRole('button',{name:'保存设置',exact:true}).click();
    await page.waitForFunction(()=>document.querySelector('#report-settings-status').textContent.includes('已保存'));
    const configAfter=await (await context.request.get(cfg.origin+'/api/reports/settings')).json();
    assert.deepEqual(configAfter.project_ids,[cfg.projectId]);assert.deepEqual(Object.keys(configAfter.activity_db_paths),[cfg.projectId]);
    for(const key of ['capture_hosts','enabled','knowledge_enabled','literature_enabled','daily_time','weekly_time','weekly_weekday','runtime'])assert.deepEqual(configAfter[key],configBefore[key]);
    await page.reload();await page.locator('#report-settings').evaluate(e=>e.open=true);await page.waitForFunction(()=>document.querySelector('#report-capture').checked);
    assert.equal(await page.locator('#report-daily-time').inputValue(),'18:00');
    checks.push('removed project settings save; preserve consent/time/runtime; no automatic schedule enablement; reload persists');
    // Already-bound settings reuse the same plan, not a per-project copy/connect flow.
    assert.equal(await page.locator('#report-copy-enable').isVisible(),false);
    await page.locator('#report-enabled').check();
    await page.locator(`input[name=report-project][value="${cfg.otherProjectId}"]`).check();
    await page.getByRole('button',{name:'保存设置',exact:true}).click();
    await page.waitForFunction(()=>document.querySelector('#report-settings-status').textContent.includes('无需重新连接'));
    const connected=await (await context.request.get(cfg.origin+'/api/reports/settings')).json();
    assert.equal(connected.connection,'configured');assert.ok(connected.project_ids.includes(cfg.otherProjectId));
    assert.deepEqual(connected.runtime,configBefore.runtime);
    assert.equal(await page.locator('#report-copy-enable').isVisible(),false);
    assert.equal(await page.locator('#report-copy-maintain').isVisible(),false);
    await page.locator('#report-daily-time').fill('19:00');
    await page.locator('.schedule-help').evaluate(e=>e.open=true);
    await page.locator('#report-copy-maintain').click();
    assert.ok((await page.locator('#report-settings-status').innerText()).includes('请先保存'));
    await page.getByRole('button',{name:'保存设置',exact:true}).click();
    await page.waitForFunction(()=>document.querySelector('#report-settings-status').textContent.includes('执行时刻已更改'));
    await page.locator('#report-copy-maintain').click();
    const instruction=await page.evaluate(()=>navigator.clipboard.readText());
    assert.ok(instruction.includes('每次运行读取最新配置'));assert.ok(instruction.includes('复用已有计划'));
    assert.ok(!instruction.includes('配置版本'));assert.ok(!instruction.includes('每天北京时间18:00'));
    // A mocked unbound receipt exercises the first-connection UI, without creating a live plan.
    await page.route('**/api/reports/settings',async route=>{
      const response=await route.fetch();const value=await response.json();
      await route.fulfill({response,json:{...value,runtime:{},connection:'pending'}});
    });
    await page.reload();await page.locator('#report-settings').evaluate(e=>e.open=true);
    await page.waitForFunction(()=>document.querySelector('#report-connection').textContent.includes('尚未连接'));
    assert.equal(await page.locator('#report-copy-enable').innerText(),'首次连接计划');
    assert.equal(await page.locator('#report-copy-enable').isVisible(),true);
    assert.equal(await page.locator('#report-copy-maintain').isVisible(),false);
    await page.unroute('**/api/reports/settings');
    checks.push('shared plan reuse; new selected projects keep receipt; first connection only; time-change warning; unsaved copy guard; no hardcoded schedule');
