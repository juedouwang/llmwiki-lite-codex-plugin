...
(async()=>{
  ...
  try{
    ...
    console.log('PASS approved two-endpoint Git icons / real local path / read-only enlarge + Escape + focus / actions follow diff under tall history');
    ...
    await page.goto(origin+'/');await page.waitForURL(origin+`/project/${pid}/todos`);
    await page.locator('.console-sidebar-brand').click();await page.waitForURL(url=>url.pathname==='/projects');
    ...
    assert.equal(new URL(fresh.url()).pathname,`/project/${wanted}/todos`);await freshContext.close();
    await page.locator(`.project-row[data-project-id="${wanted}"] .project-default`).click();
    await page.waitForFunction(()=>document.querySelector('#project-manager')?.dataset.defaultProject==='');
    await page.goto(origin+'/');assert.equal(new URL(page.url()).pathname,`/project/${pid}/todos`);
    const clearContext=await browser.newContext();const cleared=await clearContext.newPage();await cleared.goto(origin+'/');
    assert.equal(new URL(cleared.url()).pathname,`/project/${original[0]}/todos`);await clearContext.close();
    assert.equal((await (await page.request.get(origin+'/api/projects/preferences')).json()).current_project_id,currentAgent);
    console.log('PASS startup default only in a fresh browser / current project retained / brand overview / agent selection unchanged');
