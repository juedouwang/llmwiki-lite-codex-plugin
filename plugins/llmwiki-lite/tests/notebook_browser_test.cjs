...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('#nb-live').evaluate(e=>e===document.activeElement),true);
    assert.equal(await page.locator('input[type=file],.nb-plus,.nb-cell').count(),0);
    await paste(page,{text:'### 1. 编辑器\n\n**观察**：代码完成不等于硬件验证完成。\n\n```python\nx=1\n```'});
    await page.locator('#nb-preview-mode').click();await page.locator('#nb-preview h3').waitFor();assert.equal(await page.locator('#nb-preview strong').innerText(),'观察');
    ...
    await page.waitForFunction(()=>document.querySelectorAll('#nb-live img').length===1);
    assert.equal(await page.locator('#nb-live').evaluate(e=>document.activeElement===e),true);
    await paste(page,{images:1,png});await page.waitForFunction(()=>document.querySelectorAll('#nb-live img').length===2);await saved(page);
    ...
    await paste(page,{selector:'#nb-title',images:1,png});await page.waitForFunction(()=>document.querySelectorAll('#nb-live img').length===3);await saved(page);
    // Existing annotations remain visible/exportable without an add-comment toolbar.
    assert.equal(await page.locator('#nb-comment,#nb-code,#nb-save').count(),0);
    const snapshot=await (await context.request.get(api+'?format=markdown')).json();
    await context.request.post(api,{headers:{'X-Notebook-Request':'1','Origin':origin},data:{revision:snapshot.revision,document:{...snapshot.document,comments:[{id:'existing-comment',quote:'观察',text:'补充夜间场景，暂不作为最终结论。',created_at:null}]}}});
    ...
    await page.locator('#nb-edit').click();await page.locator('#nb-live').press('Control+End');
    const before=await page.locator('#nb-source').inputValue();await paste(page,{text:'\n补充记录'});await page.locator('#nb-live').press('Control+z');
    assert.equal(await page.locator('#nb-source').inputValue(),before);await page.locator('#nb-live').press('Control+Shift+z');
    ...
    await page.locator('#nb-live').press('Control+z');release();await page.waitForFunction(()=>!document.querySelector('#nb-uploads').textContent.includes('上传中'));
    await page.locator('#nb-live').press('Control+Shift+z');assert.ok(!(await page.locator('#nb-source').inputValue()).includes('<!--report-upload:'));
    assert.equal((await page.locator('#nb-source').inputValue()).match(/!\[\]/g).length,4);await page.unroute('**/notebook/upload');await saved(page);
    ...
    assert.equal((await page.locator('#nb-source').inputValue()).match(/!\[\]/g).length,5);assert.equal(pickers,0);
    for(const image of (await page.locator('#nb-source').inputValue()).matchAll(/!\[\]\(([^)]+)\)/g)) assert.match(image[1], /^\.\.\/assets\/[a-f0-9]{64}\.png$/);
    // Ordinary typing uses the textarea's native undo/redo, not whole-document resets.
    const beforeTyping=await page.locator('#nb-source').inputValue();await page.locator('#nb-live').press('Control+End');
    await page.locator('#nb-live').pressSequentially(' native typing');
    let nativeSteps=0;while(await page.locator('#nb-source').inputValue()!==beforeTyping){assert.ok(nativeSteps++<30,'native undo must reach the insertion boundary');await page.locator('#nb-live').press('Control+z');assert.ok((await page.locator('#nb-source').inputValue()).startsWith(beforeTyping),'native undo must not remove an earlier screenshot');}
    for(let i=0;i<nativeSteps;i++)await page.locator('#nb-live').press('Control+Shift+z');assert.equal(await page.locator('#nb-source').inputValue(),beforeTyping+' native typing');await saved(page);
    ...
    await other.locator('#nb-title').fill('另一页面的标题');await saved(other);await page.locator('#nb-title').fill('本页未合并标题');await page.keyboard.press('Control+s');
    ...
    await page.route('**/api/**',route=>route.abort());await page.locator('#nb-title').fill('离线恢复标题');await page.keyboard.press('Control+s');
    ...
    await page.locator('#nb-live').press('Control+End');const beforeMulti=await page.locator('#nb-source').inputValue();
    ...
    assert.equal((await page.locator('#nb-source').inputValue()).match(/!\[\]/g).length,7);
    await page.locator('#nb-live').press('Control+z');assert.equal(await page.locator('#nb-source').inputValue(),beforeMulti);
    await page.locator('#nb-live').press('Control+Shift+z');assert.equal((await page.locator('#nb-source').inputValue()).match(/!\[\]/g).length,7);await saved(page);
    ...
    await page.locator('#nb-live').evaluate(e=>{const r=document.createRange();r.setStart(e.firstChild,2);r.setEnd(e.firstChild,7);const s=getSelection();s.removeAllRanges();s.addRange(r);});
    ...
    assert.equal(await page.evaluate(()=>getSelection().toString()),(await page.locator('#nb-source').inputValue()).slice(2,7));assert.equal((await (await context.request.get(api)).json()).revision,stable);
    for(const scheme of ['light','dark']){await page.emulateMedia({colorScheme:scheme});for(const width of [1280,1024,736,580,375,320]){
      await page.setViewportSize({width,height:960});await page.waitForFunction(()=>{const e=document.querySelector('#nb-live');return e.scrollHeight<=e.clientHeight+2;});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),`overflow ${width} ${scheme}`);
