...
(async()=>{
 ...
 try {
  ...
  // Both entry points change context without leaving the project browser.
  await page.locator(`.project-row[data-project-id="${a}"] .project-row-main`).click();
  await page.waitForURL(`${origin}/projects?context=${a}`);
  assert.equal(await page.locator('body').getAttribute('data-workbench-page'),'home');
  assert.equal(await selected(),a);
  await page.locator('.console-sidebar-brand').click();await page.waitForURL(url=>url.pathname==='/projects');
  await page.locator('.console-project-switcher summary').click();
  await page.locator(`.console-project-menu a[data-project-id="${z}"]`).click();
  await page.waitForURL(`${origin}/projects?context=${z}`);
  assert.equal(await page.locator('body').getAttribute('data-workbench-page'),'home');
  assert.equal(await selected(),z);
  await page.locator('.console-sidebar-brand').click();await page.waitForURL(url=>url.pathname==='/projects');
