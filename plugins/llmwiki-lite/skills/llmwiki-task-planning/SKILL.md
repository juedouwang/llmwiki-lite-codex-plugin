...
任务完成后的异步收口：如果本轮确实完成了一个有价值的科研交付，最终回复可按 `llmwiki-research-record` Skill 的隐藏标记协议追加收口标记。已有每日待办时填入真实 `task_id` 和交付摘要，Hook 会异步提交为“待验收”；没有每日待办但产生了独立科研认识时不要编造 task_id，让它作为独立科研记录保存。普通解释、失败尝试和未完成计划不要追加标记。
