...
1. 先取得用户明确选择：项目范围、日报时刻、周报星期/时刻及知识、文献、对话取材开关。用户可在网页保存，也可在对话中授权助手通过源码设置接口保存；不要求重复确认。日报、周报均为工作台独立的多项目文档，不设置归档项目。只有用户明确要求“全部项目”才使用当时注册项目的完整快照；以后新增项目不自动加入授权，不暗填时刻。`status` 会返回配置、`revision` 与 `config_path`；启用指令里的 revision 过时应重新确认范围。
...
- 对话仅来自用户授权后、按注册项目目录归属的本机会话；先筛选索引元数据再读命中的消息。排除配置线程、自动任务和其他项目；索引格式变化就报告缺口，不降级全账户扫描。
- 当前内置取材适配器仅支持已验证的一类本机宿主日志；其他宿主、网页聊天、图片内文字并未自动接入。已有手写笔记和已保存科研记录仍可取材。不能声称所有助手已覆盖。
- Git 提交/差异只读、无窗口；历史工作区状态不能据今天的 diff 编造。变更提示仅是线索，不是任务完成证明。限量/超限/无授权都在来源缺口展示。
