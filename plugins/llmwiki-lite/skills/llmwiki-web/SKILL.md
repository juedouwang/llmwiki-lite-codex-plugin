...
不显示常驻手动保存、插入代码或添加批注按钮；新内容直接使用 Markdown，旧批注和引用快照仍保留。图片在编辑状态直接显示，上传成功不额外显示“截图”标签；标题输入立即更新。图片画笔标注不在此版本。创建/修改时间只在“更多 → 笔记信息”查看，不进入正文；标签、历史、另存和 Markdown 复制在更多菜单。截图上传失败保留重试，未完成上传不允许导出/正式确认。多窗口冲突或本机恢复稿需要明确选择，不能用 MCP 强制覆盖规避冲突。
...
进度页保留“继续上次”、Todo/Done及任务排期。默认从今天起显示一周，可切换两周/四周、前后窗口，点击“今天”回到当前日期。有截止日的未完成任务显示今天至截止日的纯色跨度条，不加深色左边框，跨窗口裁剪但提示保留真实日期；今天截止为单日条，逾期显示在今天列并提示天数。横向滚动时任务名称固定；浏览时间范围不回写任务日期或状态，未排期留在 Todo，已完成不进入排期。

点击“新建任务”打开任务表单；列表按优先级排序，圆圈显式完成/恢复。任务编辑统一描述、可选截止日（DDL）和高/中/低优先级，不要求开始日期或进行中/卡住状态。优先级与日期桌面两列对齐，快捷日期跟随日期字段，窄屏单列。描述可直接 Ctrl+V 截图；旧任务两栏、排期和自动上下文无损兼容，人工描述不被自动摘要覆盖。旧记录中的待办须选择导入，不自动生成日期或推断完成。人工任务在 `.research-progress/tasks.json`，自动上下文在 `.research-progress/contexts.json`。并发冲突须处理后保存。自动整理未接通时只说明尚未接通，不要启动模型或终端，也不要把注入摘要说成自动化已完成。
...
报告默认配置关闭，但用户可已明确授权并绑定内置计划：必须读真实设置/回执判断，不用“默认关闭”否认当前启用状态。网站不推理，Hook 不掌握全部聊天，文件改变不代表任务完成；宿主离线不保证报告按时生成。源码更新需重启旧网页进程，不能改缓存冒充发布，也不擅自暂停/重置已有计划。保存配置会跳过已注销项目的旧授权清理，不访问其保留文件；新请求显式选择已注销项目则提示刷新并保持配置不变。旧配置存在这类引用时，正常重启服务后在设置页重新保存即可，不代替用户改动授权或创建计划。
...
项目「代码」页只操作注册仓库根目录及其 Git 登记、双向绑定验证通过的已有工作树：真实版本图、选文件本地保存、建/切分支、普通合并、有限 UTF-8 冲突处理与恢复为新提交。恢复不抹去旧历史，未选已暂存文件不夹带进提交；缺作者身份可页内补填且只写本仓库。每次写入须由用户明确确认，预览失效必须重新查看，不允许替用户丢弃文件或自动重试。

代码页顶部按钮使用小写 `pull` / `push`，不改变检查/确认/执行流程；代码栏图标回到批准 HTML 原型的原始几何，不再额外添加圆圈。右栏遵循批准的 HTML 原型，简短元数据可点击展开查看本地仓库绝对路径；diff可在只读大窗口放大查看，关闭后保留节点和文件；建分支/恢复紧跟diff而非页面末端。新增/删除行数来自真实 Git，二进制不显示虚构数字，差异保留不连续片段提示。不能把原型示意提交或行数写入生产页面。
...
网站根入口 `/` 在本次网站会话已有选择时继续当前项目；没有选择或服务重启后按指定启动项目初始化，未指定则打开排序首项，没有项目时进入 `/projects`。品牌链接始终返回 `/projects` 总览。总览中的文件夹图标支持拖动排序及键盘上下调整，星标按钮指定/取消启动项目，修改自动保存并在失败时提示、恢复排序。设置只写注册表旁 `settings.json` 的 `web_default_project_id` / `project_order`，不改变助手 `current_project_id`，不修改项目文件。显式默认优先于顺序，新注册项目在手动列表末尾，默认项目注销后回退到剩余首项。源码修改须正常重启服务才完整生效；不将刷新静态资源说成新路由已加载。

本轮项目管理：项目选择下拉仅列项目；外部点击/Escape关闭。总览行右键、Shift+F10或更多按钮可重命名/移除。移除只取消注册，重命名只改显示名。默认项目只在本次网站会话缺少当前选择时初始化；设置/项目总览不能抢占当前项目，不能修改 agent 的全局项目选择。

已有多工作树不因数量而全局只读，也不用重复注册科研项目。代码页“选择工作树”显示路径、分支、当前与注册目录；占用分支保留禁用检出并提供“进入所在工作树”。选择只改变代码页目录，不 checkout、不改注册 source_root、不迁移知识/记录，不创建树或合并 main。单树隐藏入口。

工作树 URL 带 `?worktree=<id>`，刷新与历史返回保留目录；默认代码入口返回注册目录。每次请求重新从注册仓库解析 opaque ID，不接收任意路径；所有读写/预览/合并状态绑定选中目录，共同仓库锁及锁内重校验继续生效。无效目录明确报错并提供返回注册目录，不能静默回退执行。其他目录的 dirty/index 不得混入当前保存；分支与仓库配置仍由 Git 共享。草稿与慢请求沿用离页保护，不可跨树复用预览。不得强制切换、删树或解除真实策略绕过限制。

共享自动整理计划只需首次连接。已连接时不再显示主操作区的连接按钮；新注册项目默认勾选，下次运行自动读取；不想参与可取消勾选并保存，重复注册不恢复已取消选择。仅沿用已保存的取材宿主开关，不自动启用对话采集或暂停的计划；移除项目同步退出范围，历史已注销 ID 不阻断报告。不要求按项目复制指令或创建计划。首次连接及恢复失效连接使用主按钮，高级检查和执行时刻同步在“计划与取材说明”内；设置未保存时不能复制过期指令。网页保存不修改宿主日程，不把已连接等同于生成成功。

网页 favicon 与桌面快捷方式共用 `scripts/static/workbench.ico`，不另画一套图标。修改 Python 页面模板或路由后，需正常重启既有网页服务再刷新验证；不要启动重复服务。
