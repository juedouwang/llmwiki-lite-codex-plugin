...
科研进度默认显示本周（周一至周日），可切换两周/四周科研时间轴。点击“新建任务”展开输入，回车添加，取消收起；列表按优先级排序并支持显式完成/恢复；点击任务编辑统一描述、可选截止日（DDL）和高/中/低优先级，不再要求开始日期或进行中/卡住状态。描述可直接 Ctrl+V 截图，未排期单列，已完成默认折叠；旧任务两栏与自动上下文无损兼容，人工描述不被自动摘要覆盖。旧记录中的待办须选择导入，不自动生成日期或推断完成。人工任务在 `.research-progress/tasks.json`，自动上下文在 `.research-progress/contexts.json`。并发冲突须处理后保存。自动整理未接通时只说明尚未接通，不要启动模型或终端，也不要把注入摘要说成自动化已完成。
...
项目「代码」页只操作注册仓库或已注册的 linked worktree 根目录：真实版本图、选文件本地保存、建/切分支、普通合并、有限 UTF-8 冲突处理与恢复为新提交。恢复不抹去旧历史，未选已暂存文件不夹带进提交；缺作者身份可页内补填且只写本仓库。每次写入须由用户明确确认，预览失效必须重新查看，不允许替用户丢弃文件或自动重试。
...
已有多工作树不因数量而全局只读；网页操作仅作用于当前注册树，共同仓库锁保护写入。另一工作树使用中的分支必须说明原因并禁用切换。不得强制切换、删树或解除真实策略来绕过限制。
