...
def editor_markup(prefix: str, title: str, back: str, *, notebook=False) -> str:
    ...
    heading = (f'<input id="{ident("title")}" class="rw-title-input" aria-label="笔记标题" placeholder="无标题" maxlength="200" autocomplete="off">'
               if notebook else f'<h1 class="rw-document-title">{esc(title)}</h1>')
    primary = (f'<button id="{ident("save")}" class="rw-button rw-primary">保存</button>' if notebook else
               f'<button id="{ident("confirm")}" class="rw-button rw-primary">确认为正式版</button>')
...
<span id="{ident("state")}" class="muted"></span><span id="{ident("save-state")}" class="muted" role="status" aria-live="polite">正在打开…</span>
...
<div class="rw-editor-header"><button id="{ident("edit")}" class="rw-button" aria-pressed="false">编辑</button><button id="{ident("preview-mode")}" class="rw-button" aria-pressed="true">预览</button><span class="rw-spacer"></span><button id="{ident("code")}" class="rw-button">代码</button><button id="{ident("comment")}" class="rw-button">批注</button>{'' if notebook else '<button id="'+ident('sources')+'" class="rw-button">来源</button>'}</div>
