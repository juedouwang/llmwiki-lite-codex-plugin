(() => {
  ...
  let writes = 0, allowNavigation = false;
  const formGuards = [];
  ...
  function remember() {if(!active())return;sessionStorage.setItem(key, JSON.stringify({url:location.pathname+location.search, scroll:scrollY}));}
  ...
  if (back) {
    ...
    try {const saved=JSON.parse(sessionStorage.getItem(key)); if(saved?.url===location.pathname+location.search) requestAnimationFrame(()=>{if(active())scrollTo(0,saved.scroll);});} catch {}
  }
  ...
  async function refresh(url=location.href, highlight) {
    ...
    root.querySelectorAll('[data-item]').forEach(a=>a.addEventListener('click',remember));
    if(highlight) {const row=root.querySelector('#lit-'+highlight);row?.classList.add('literature-highlight');if(active())row?.scrollIntoView({block:'nearest'});}
  }
  let timer, searchPending=false;
  async function search() {
    if(!active())return;
    searchPending=false;
    const value=root.querySelector('#literature-search').value;
    const url=new URL(base,location.origin);if(value)url.searchParams.set('q',value);
    history.replaceState(history.state,'',url);try {await refresh(url);}catch(e){message.textContent=e.message;}
  }
  root.querySelector('#literature-search')?.addEventListener('input',event=>{
    if(event.isComposing) return;
    searchPending=true;clearTimeout(timer);timer=setTimeout(search,180);
  });
  root.querySelectorAll('[data-literature-form]').forEach(form=>{
    let composing=false, busy=false, requestId=uid();
    const fingerprint=()=>JSON.stringify([...form.querySelectorAll('input,textarea,select')].map(e=>[e.value,e.checked]));
    const baseline=fingerprint();
    formGuards.push(()=>busy||(!form.hidden&&(composing||fingerprint()!==baseline)));
    ...
    form.addEventListener('submit',async event=>{
      ...
      try {
        ...
        if(kind==='edit') {article.dataset.revision=result.item_revision;if(active()){allowNavigation=true;location.replace(location.pathname);}return;}
        if(kind==='import') {if(active()){allowNavigation=true;location.href=base;}return;}
        ...
        root.querySelector('#literature-search').value='';if(active())history.replaceState(history.state,'',base);await refresh(base,result.item_id);
      } catch(e) {error.textContent=e.message; if(active())(focused && form.contains(focused) && focused!==button ? focused : form.querySelector('input,textarea'))?.focus();}
  ...
  root.querySelector('[data-remove]')?.addEventListener('click',async event=>{
    ...
    try {await post(`item/${article.dataset.itemId}/delete`,{expected_item_revision:article.dataset.revision});if(active()){allowNavigation=true;location.href=back.href;}}
  ...
  details?.addEventListener('toggle',status);window.addEventListener('focus',status,{signal:lifecycle.signal});
  const unsaved=()=>writes>0||formGuards.some(check=>check());
  document.addEventListener('workbench:before-leave',event=>{
    if(!owns(event)||!active()||!unsaved())return;
    event.preventDefault();message.textContent=writes?'请等待当前操作完成。':'尚未保存，请先完成或取消当前修改。';
  },{signal:lifecycle.signal});
  window.addEventListener('beforeunload',event=>{if(active()&&!allowNavigation&&unsaved()){event.preventDefault();event.returnValue='';}},{signal:lifecycle.signal});
  document.addEventListener('workbench:leave',event=>{if(owns(event))clearTimeout(timer);},{signal:lifecycle.signal});
  document.addEventListener('workbench:enter',event=>{if(owns(event)&&active()){if(searchPending)search();status();}},{signal:lifecycle.signal});
  document.addEventListener('workbench:dispose',event=>{if(owns(event)){clearTimeout(timer);searchGeneration++;lifecycle.abort();}},{signal:lifecycle.signal});
