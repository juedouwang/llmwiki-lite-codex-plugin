...
- “科研进度”：从今天起的一周/两周/四周任务跨度、未排期、Todo/Done 和继续上次。任务统一描述、一个可选截止日期（DDL）、高/中/低优先级；高优先级在前，柔和红/黄/绿区分，完成/恢复明确点击。名称、截止日、优先级和完成情况只由用户修改，不因文件变化或午夜自动完成。
...
3. 仅首次点击“首次连接计划”复制指令交给宿主。已连接后该按钮收起，新注册项目默认勾选并纳入下次整理，不想参与时取消勾选并保存即可；重复注册不恢复用户已取消的选择，不按项目另建任务、不重复绑定。默认参与只继承已保存的取材开关，不自动开启对话采集或已暂停计划；注销项目同步移出范围，旧配置的注销 ID 不阻断整轮报告。高级维护入口在“计划与取材说明”中；更改执行时刻需同步现有计划。按 `skills/llmwiki-research-record/SKILL.md` 使用官方计划工具复用同一个 heartbeat；获得真实 id/目标线程后执行源码 CLI 的 `bind`，只读官方回执并核对 ACTIVE。已有任务必须优先复用，无论当前是否暂停，不能另起第二个后台。
...
同一个科研项目可管理注册仓库的已有工作树，无需把每个 linked worktree 注册成新项目。代码页有多个工作树时显示“选择工作树”，列出分支、目录路径、当前/注册目录；分支被占用时仍禁止重复检出，但提供“进入所在工作树”。**进入工作树只是换一个目录查看和操作，不是切分支，也不执行 checkout。**

URL 的 `?worktree=<id>` 保留当前目录，刷新/后退不会串树；从其他栏目点击默认代码入口回到注册目录。只有同仓库 Git 登记并通过双向绑定核验的目录可选；移走/失效后明确报错，可返回注册目录，绝不静默转向其他目录执行。单工作树不显示额外入口。

所有读写与预览绑定选中工作树，保留共同 Git 目录锁和锁内重校验；合并状态/远端检查记录按目录隔离。选目录不改变注册表、科研记录和任何目录的未提交内容；保存仍须预览确认，其他树未提交/暂存内容不受影响。分支引用和仓库配置仍由 Git 共享；不自动创建工作树、合并 main 或解除占用。子模块、LFS/自定义过滤器、稀疏/部分克隆等仍限浏览。机器状态需在仓库外或被忽略；网页不会修改 `.gitignore`。Git 不触发任务完成、日报/知识/文献推理，真实自动计划保持原配置。
...
打开网站根地址 `/`（包括桌面快捷方式）始终进入跨项目的 `/daily` 每日待办，不因上次项目或默认项目进入科研进度。点击「野人工作台」打开 `/projects` 项目总览；项目浏览页点击项目名称或切换器只更换当前项目、仍留在项目浏览页；在具体栏目使用切换器则留在同一栏目。项目浏览页不再显示仅部分项目拥有的进行中任务副标题。仅明确点击「科研进度」才进入任务页面。

在总览拖动项目前的文件夹图标即可排序，松开自动保存；键盘聚焦文件夹后可用上下方向键调整。点击项目右侧星标指定默认项目，再点取消；这只影响未选项目时的侧栏默认上下文，不改变每日待办首页。设置保存在本机注册表旁的 `settings.json`（`web_default_project_id`、`project_order`），不写入项目源码，也不改助手使用的 `current_project_id`。未手动排序时保留原名称排序；手动排过后新项目追加在末尾，取消注册默认项目后回到剩余第一项。保存失败回退本次排序并明确提示，不假装保存成功。跨栏目缓存里的项目菜单同步顺序。

隔离验证：`python -B -m unittest discover -s plugins/llmwiki-lite/tests -p test_project_preferences.py`；`python -B plugins/llmwiki-lite/tests/run_site_browser.py --refinements` 覆盖拖动、默认入口、真实Git元数据及diff放大。网页没有账户登录；浏览器自行恢复的具体深链接仍打开原页面，桌面快捷方式和根地址则进入每日待办。
...
- 任务描述支持 Ctrl+V 截图与 Markdown，不再要求手选开始时间或“进行中/卡住”。排期默认从今天起，显示今天至截止日的柔和纯色跨度条，不加深色左边框；跨周裁剪，逾期在今天列提醒。切换时间窗口不改变任务数据。优先级/日期对齐，日期快捷按钮跟随日期控件，窄屏自动单列。旧任务两栏和排期数据仍保留、兼容读取，不会自动将任务标成完成。
- 笔记、日报/周报以自动保存为主，不再显示常驻保存、代码、批注插入按钮；编辑时看得到图片，标题输入立即更新。既有批注/历史不删除。报告正式确认仍需主动点击。
- 自动整理设置展开说明：计划决定“何时整理”，项目会话关联决定“用哪些资料”。历史配置中的已注销项目不再阻断保存；重新保存会按当前选择去掉旧引用，但不访问其保留文件。旧页面新提交无效项目则提示刷新，保留原配置。保存设置不创建后台进程、不自动连接计划；是否已连接以真实状态为准。源码后端修正须正常重启服务后生效。
...
日报/周报的 Git 材料仅含提交说明、时间与版本标识；不读取 diff、工作区状态、变化路径或未跟踪文件正文。不影响代码页 diff 或知识库。笔记与报告的 Markdown 预览统一左对齐及标题间距。

来源包 `attachments` 列出授权文字中明确引用的本地图片（仅候选元数据，不理解图像）。助手按文字上下文选择关键图，通过 `![说明](report-image:<id>)` 引用并提交所属来源 ID；finish 将选中的 PNG/JPEG/GIF/WebP 复制到工作台 `records/assets`，单张不超过 10 MB，同内容去重。临时截图删除后已保存的报告图仍可显示。不可用的选中图片留占位和缺口，不阻塞文字保存。未有本地引用的内嵌图片不在当前支持范围。
