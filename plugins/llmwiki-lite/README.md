...
共享侧栏按“科研进度、科研记录、日报与周报、知识库、文献、代码”排列，项目切换保持栏目；报告仍是工作台文档。连续笔记和报告复用同一交互，业务存储保持独立。原型原稿保存于 `openspec/changes/research-daily-weekly-reports/reference/approved-workbench.html`。共享壳接入不代表进度周轴、知识双栏和完整 Git 交互均已完成，具体以各 change 的任务/验证记录为准。
...
「拉取」先选择已有远端/分支并检查，再确认更新代码；检查本身不修改工作区。「上传」再次显示固定分支、远端地址与本地缓存下的提交清单，只普通上传该分支已保存的版本，不上传未提交文件或其他分支/标签。没有自动联网、强推、自动重试或终端弹窗；认证/自定义 Hook/签名等不支持策略应在原工具配置后重试，不偷偷绕过。

多工作树、子模块、LFS/自定义过滤器、稀疏/部分克隆等暂限浏览。机器状态需在仓库外或被忽略；网页不会修改 `.gitignore`。Git 不触发任务完成、日报/知识/文献推理，真实自动计划保持原配置。

隔离验证：`python -B plugins/llmwiki-lite/tests/run_code_browser.py`（需已有 Playwright/Chrome；可用 `LLMWIKI_PLAYWRIGHT` 指定模块路径）。验收及未完成项见第五份 OpenSpec `verification.md`；本地开发实现不等于 GitHub/市场/缓存已发布。
