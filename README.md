...
原待办升级为持久化科研时间轴，并记录“上次做到哪 / 下一步”；笔记支持更直接的截图粘贴、拖放与连续输入。旧待办需显式导入，不改原日档。自动日报/周报尚未启用，需先接入获授权的项目对话及定时运行端；边界与全站交互审查见 `docs/research-continuity.md`。

### 科研工作台精简实施范围（待实施）

[精简执行计划](docs/research-workbench-mvp.md) 优先打通科研进度、日常留痕、自动日报周报与知识整理，再补齐文献和可操作 Git；共 6 项任务、8 项核心验收。上一版完整规格只作参考，不再作为首版全量交付门槛。计划不代表功能已完成或自动任务已开启。
